...
function bestTidalMatch(tracks, title, artist) {
  ...
  for (const t of tracks) {
    // title weighted 35%, artist weighted 65% — prevents wrong-artist matches
    const score = strScore(t.title, title) * 0.35 + strScore(t.artist, artist) * 0.65;
    if (score > bestScore) { bestScore = score; best = t; }
  }
  console.log(`[tidal-match] best score ${bestScore.toFixed(3)} → "${best?.title}" by "${best?.artist}"`);
...
app.get('/health', async (req, res) => {
  ...
  res.json({ status: claudoOk ? 'ok' : 'degraded', claudochrome: claudoOk, error, version: '5.4.0' });
});

// ─── Start ────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`SpotiFLAC v5.4.0 → http://localhost:${PORT}`);
